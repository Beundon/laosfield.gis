...
export default function FieldHud({ liveGps, gpsError, isIct }: FieldHudProps) {
  ...
  // Current-position status: searching (no fix yet) / weak (poor accuracy) /
  // fixed (good accuracy) / unavailable (permission denied or no hardware).
  let statusClass = 'field-hud__status--searching';
  let statusLabel = 'Searching for GPS...';
  if (liveGps) {
    const weak = liveGps.accuracyMeters != null && liveGps.accuracyMeters > 30;
    statusClass = weak ? 'field-hud__status--weak' : 'field-hud__status--fixed';
    statusLabel = weak ? `Weak fix ${accuracy}` : `Position fixed ${accuracy}`;
  } else if (gpsError) {
    statusClass = 'field-hud__status--off';
    statusLabel = 'GPS unavailable';
  }

  return (
    <div className="field-hud" role="status" aria-label="Field coordinate readout">
      <div className={'field-hud__status ' + statusClass}>
        <span className="field-hud__status-dot" />
        {statusLabel}
      </div>
      ...
      {gpsError && !liveGps && <div className="field-hud__error">GPS: {gpsError}</div>}
    </div>
  );
}
